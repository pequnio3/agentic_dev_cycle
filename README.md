...
Add as a submodule to your project, then run the install script:

```bash
git submodule add https://github.com/<you>/agentic-dev-cycle .agentic-dev-cycle
bash .agentic-dev-cycle/install.sh
```

The install script:
- Symlinks `.claude/skills/` → `<absolute path>/skills/` so slash commands appear in Claude Code
...
This asks you seven questions about your project (tech stack, test commands, architecture
patterns, model preferences) and generates:
...
`.dev_cycle/` is gitignored by default — your workflow state is private to you.
Teams who want shared workflow state: remove `.dev_cycle/` from `.gitignore` and commit it.

Commit everything to main so build agents (which run in worktrees) can find it:
...
### Updating the workflow after pulling submodule changes

```bash
git submodule update --remote .agentic-dev-cycle
bash .agentic-dev-cycle/install.sh   # safe to re-run, skips existing config files
```
...
```
.agentic-dev-cycle/          ← this submodule (workflow code, don't edit)
  skills/                    ← slash command definitions
  dev_cycle/                 ← templates and generic docs (source)

.claude/
  skills -> <absolute path>/skills   ← symlink (created by install.sh)

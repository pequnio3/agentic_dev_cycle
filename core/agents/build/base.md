...
- `.dev_cycle/project.yaml` — tech stack, architecture patterns, implementation phases, gate commands
...
2. Break into numbered tasks by phase — use **implementation_phases** from `.dev_cycle/project.yaml`
...
each requirement and each scenario has a corresponding task. If any single phase would
...
Both code and tests must be committed together. Gate must be clean before the next phase.

**If Scenarios are present:** After each phase, run the BDD framework to check scenario
progress. Note which scenarios are now passing. All scenarios must pass before Step 7.

**Constraints:**
- Follow the architecture patterns in `.dev_cycle/project.yaml` exactly
...
- **Correctness validation** — fill in automated checks, manual/scenario checks, and evidence; check the boxes when done
...
- **Prereqs:** branch checked out, env vars, seed data, local URLs/ports (from `.dev_cycle/project.yaml` if relevant)
...
Base them on: models from `.dev_cycle/project.yaml`, count of manifest files touched, phases,

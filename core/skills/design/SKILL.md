...
3. Spawn a **foreground** design agent in an isolated worktree:
   ...
   Agent tool:
     ...
     prompt: |
       Read `.dev_cycle/agents/design/base.md` and `custom.md` (merged at runtime — personality in `custom.md`).
...
7. **Create GitHub Issues** (automatic — no user action needed):
   ...
   **Create the issue** using `core/issue_template.md` (tool repo) or `.dev_cycle/issue_template.md` (after install) as the body structure:
   ```bash
   gh issue create \

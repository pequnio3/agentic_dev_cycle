...
5. Read `.dev_cycle/project.md` for the build model (Models table; default: `claude-opus-4-6`).
...
3. Read `.dev_cycle/project.md` for the build model.
4. **Series loop:**
...
2. **Chain `/review`** — run the **`review` skill** for this issue. Pass:
   ...
   - `PR: <URL>`
   Use the review model from `.dev_cycle/project.md`. **One review at a time.** Let the review subagent **finish completely** before any **next** step (e.g. spawning the **next** series build).

...
Read `.dev_cycle/project.md` to understand how to start the project's dev servers,
...
Check `.dev_cycle/project.md` for the correct dependency install commands.
...
Free the ports before starting. Check `.dev_cycle/project.md` for which ports the
project uses, then:
...
Use the start commands from `.dev_cycle/project.md`.

After launching, verify startup:
- Wait a few seconds for startup
- Hit the health check endpoint (see `project.md`)
